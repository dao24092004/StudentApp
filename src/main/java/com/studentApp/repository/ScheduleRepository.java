...
public interface ScheduleRepository extends JpaRepository<Schedule, Long> {
    ...
    @Query("SELECT s FROM Schedule s WHERE s.teacher.id = :teacherId AND DATE(s.startTime) = :date")
    List<Schedule> findByTeacherIdAndDate(@Param("teacherId") Long teacherId, @Param("date") LocalDate date);

    @Query("SELECT s FROM Schedule s WHERE s.teacher.id = :teacherId AND DATE(s.startTime) BETWEEN :startDate AND :endDate")
    List<Schedule> findByTeacherIdAndWeek(@Param("teacherId") Long teacherId, @Param("startDate") LocalDate startDate,
            @Param("endDate") LocalDate endDate);

    @Query("SELECT s FROM Schedule s JOIN s.classEntity c JOIN c.registrations r WHERE r.student.id = :studentId AND DATE(s.startTime) = :date")
    List<Schedule> findByStudentIdAndDate(@Param("studentId") Long studentId, @Param("date") LocalDate date);

    @Query("SELECT s FROM Schedule s JOIN s.classEntity c JOIN c.registrations r WHERE r.student.id = :studentId AND DATE(s.startTime) BETWEEN :startDate AND :endDate")
    List<Schedule> findByStudentIdAndWeek(@Param("studentId") Long studentId, @Param("startDate") LocalDate startDate,
            @Param("endDate") LocalDate endDate);

    @Query("SELECT s FROM Schedule s JOIN s.classEntity c JOIN c.registrations r WHERE r.student.id = :studentId AND s.semester.id = :semesterId")
    ...
    @Query("SELECT s FROM Schedule s JOIN s.classEntity c JOIN c.registrations r WHERE r.student.id = :studentId AND s.semester.id = :semesterId AND s.week = :week")
    ...
    @Query("SELECT s FROM Schedule s WHERE s.teacher.id = :teacherId AND s.semester.id = :semesterId AND s.week = :week")
